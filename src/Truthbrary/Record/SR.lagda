...
instance
  ...
  readMayb {_} {A} = record {readMaybe = Q ∘ toList}
    ...
    Q t = if justice then just (t' >>= readMaybe) else nada
      where
      justice = "just " ≡ᵇ fromList (Data.List.take 5 t)
  ...
  readSum {_} {_} {A} {B} = record {readMaybe = d}
    where
    apf = flip $ λ f → fromList ∘ f ∘ toList
    inj : ∀ {a b} → {A : Set a} → {B : Set b}
        → ⦃ Read A ⦄
        → (A → B) → String → Maybe B
    inj f q = unparens (apf q $ Data.List.drop 5) >>= mapₘ f ∘ readMaybe
    d : String → Maybe $ A ⊎ B
    d q with apf q $ Data.List.take 5
    ... | "inj₁ " = inj inj₁ q
    ... | "inj₂ " = inj inj₂ q
    ... | _ = nothing
\end{code}
