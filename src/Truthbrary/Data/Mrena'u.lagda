...
\begin{code}
  module _+_ where
    ...
    r≡f+z : (s : Sign)
          → (n : ℕ)
          → fromℝ- s n ≗ (λ f → from𝔻 s f + fromℤ (s ℤ.◃ n))
    r≡f+z = {!!}
...
\begin{code}
  module _-_ where
    ...
    r-s≈r'-s : {r r' s : ℝ} → r ≈ r' → (r - s) ≈ (r' - s)
    r-s≈r'-s {s = s} = (r-s≈r'-s' {u = s} ⍨) _≈_.r≈r
    ...
    r≥r-s : (r : ℝ) → _≥ fromℕ 0 ⊆′ r ≥_ ∘ r -_
    r≥r-s r = _⊎_.map ∘ r≈r-0 r ˢ r>r-s r
    ...
    inv r = _≈_.≈⇒≈⍨ $ begin
      (fromℕ 0 - r) + r ≈⟨ _+_.+≈+⍨ (fromℕ 0 - r) r ⟩
      r + (fromℕ 0 - r) ≈⟨ 0≈r+[0-r] r _ _≈_.r≈r ▹ _≈_.≈⇒≈⍨ ⟩
      fromℕ 0 ∎
      where
      open import Relation.Binary.Reasoning.Setoid _≈_.setoid
\end{code}
      ...
      R = R[r]∧r≈s⇒R[s] (Fromℕ.fromℕ-Rational 2) d
        ...
        d = _≈_.≈⇒≈⍨ $ begin
          ...
          ½+½≈1 = begin
            ...
            _ ≈⟨ _+_.ℕ+ 1 1 ▹ _≈_.≡⇒≈ ▹ r/s≈r'/s {fromℕ 1 + fromℕ 1} N ⟩
            f (fromℕ 2) ≈⟨ r/r≈1 _ N ⟩
            fromℕ 1 ∎
            where
            N = Fromℕ.fromℕ[s]≉0 1
...
\begin{code}
  module Frinu where
    ...
    -r/-s<-r : (r s : ℝ)
             → (z : s > fromℕ 1)
             → ¯_ r > frinu (¯ r) (¯ s) (S.r>1⇒r≉0 z ∘ S.¯r≈0⇒r≈0 _)
    -r/-s<-r = {!!}
    ...
    ℚ/' z n = begin
      fromℚ (z ℚ./ ℕ.suc n) ≈⟨ _≈_.r≈r ⟩
      _ ≈⟨ k ▹ Relation.Binary.PropositionalEquality.cong fromℚ ▹ _≈_.≡⇒≈ ⟩
    ...
    R[ℕ/ℕ] m n = (ℚ.fromℤ (ℤ.+ m) ℚ.÷ n'++ ,_) ∘ ℕ/' m n
      where
      n'++ = ℚ.fromℤ $ ℤ.+ ℕ.suc n
    ...
    R[ℤ/ℕ] : (z : ℤ)
           ...
           → let N = Fromℕ.fromℕ[s]≉0 n in
             Rational $ frinu (fromℤ z) (fromℕ $ ℕ.suc n) N
...
\begin{code}
  module _^_ where
    ...
    ∃R[I^R] = (√2 , fromℕ 2) , ({!!} , R[ℕ] 2) , 2ℚ , √2^2≈2
      ...
      √2^2≈2 = begin
        (√2 ^ fromℕ 2) ≈⟨ _≈_.r≈r ⟩
        ((fromℕ 2 ^ frinu (fromℕ 1) _ N) ^ fromℕ 2) ≈⟨ _≈_.r≈r ⟩
    ...
    R[ℕ^ℕ] m n = (ℚ.fromℤ ∘₂ ℤ.+_ ∘₂ ℕ._^_) m n , d m n
      where
      d = λ m n → begin
        fromℕ m ^ fromℕ n ≈⟨ ℕ^ m n ⟩
        fromℕ (m ℕ.^ n) ≈⟨ Fromℕ.fromℕ≡fromℤ∘ℤ+_ _ ▹ _≈_.≡⇒≈ ⟩
        fromℤ (ℤ.+ (m ℕ.^ n)) ≈⟨ fromℤ-fromℚ _ ▹ _≈_.≡⇒≈ ⟩
...
\begin{code}
  module _>_ where
    ...
    s<r+s r s z = r>s⇒r'>s' _ s _ _ s+r>s +⍨ _≈_.r≈r
      where
      s+r>s = r<r+s s r z
      +⍨ = _+_.+≈+⍨ s r
...
\begin{code}
  module ∣_∣ where
    ...
    r≥0⇒r≈∣r∣ r = _⊎_.[_,_] 0≈∣0∣ $ _≈_.≡⇒≈ ∘ +r≡∣+r∣ _ ∘ r>0⇒s[r]≡+ r
      where
    ...
    I[∣r∣]⇒I[r] = jonis {!!} {!!}
      where
      jonis = SignV.jonis {P₁ = Irrational ∘ ∣_∣}
